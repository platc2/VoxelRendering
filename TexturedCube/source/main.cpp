...
struct ApplicationSettings
{
	...
	/** Window title */
	static constexpr char* const k_szWindowTitle = "TexturedCube";
...
int main()
{
	...
	RenderPass xPass(strVertexShaderSource, strFragmentShaderSource, vecAttributes, { xModelMatrix, xCubeColor, xTextureSwitch }, { xFloorTexture, xCubeTexture }, {});
	...
	glfwSetKeyCallback(pxWindow, xKeyPressed);
	...
	*static_cast<glm::vec3*>(pCubeColorMemory) = glm::vec3(0.75f);
	xCubeColor.unmap();
	void* const pTextureSwitchMemory = xTextureSwitch.map();
	while(not glfwWindowShouldClose(pxWindow))
	{
		...
		*static_cast<std::int32_t*>(pTextureSwitchMemory) = xData.switchValue;

		xPass.display();
		glBindVertexBuffer(0, xVertexBuffer.m_uiHandle, 0, sizeof(float) * 3);
		glBindVertexBuffer(1, xVertexBuffer.m_uiHandle, sizeof(float) * 72, sizeof(float) * 2);
